...
<p align="center">
  <strong>静默守护 — Android 短信、位置与监护告警助手</strong>
</p>
...
SilentGuard 是一个开源的 Android 个人设备记录与监护告警助手。应用在用户授权后记录短信、位置轨迹和邮件发送状态，并按配置发送到指定邮箱，适用于个人设备管理、老人儿童监护、家庭设备协助等场景。

## ✨ 功能特性

- **短信记录** — 在用户授权后记录收到的短信，并发送邮件提醒
- **位置记录** — 按设定间隔定时获取坐标，支持高精度/省电定位模式
- **异常未定位告警** — 连续超过阈值无法成功获取定位时发送邮件提醒，辅助发现权限、后台运行或定位服务异常
- **一键求助** — 首页 SOS 按钮可立即发送当前位置、设备型号、时间和地图链接到监护邮箱
- **低电量提醒** — 定位轮询时顺带检查电量，低于 10% 自动发送邮件提醒，避免设备关机后失联
- **每日位置汇总** — 每天 23:59 自动发送当天轨迹汇总，即使当天没有新轨迹点也会发送状态邮件
- **智能省电** — 设备静止时自动延长定位间隔（最大2倍），移动时立即恢复；夜间自动切换低功耗定位
- **轨迹地图** — 轨迹页打开时可立即记录当前位置，并自动定位到最新轨迹点
- **位置去重** — 新位置与最近记录距离不足 100 米时自动跳过，减少重复数据
- **地图邮件** — 配置高德 Web API Key 后，位置邮件包含静态地图、路径标记和地址信息
- **设备识别** — 位置邮件、告警邮件和求助邮件标题包含手机品牌与型号，便于多设备区分
- **发送记录** — 记录邮件发送成功/失败状态，可在记录页按类型筛选查看
- **后台保活** — 多重保活机制（前台服务 + AlarmManager 兜底 + onTaskRemoved 重启 + 开机自启）
- **国产 ROM 适配** — 首次开启守护时自动引导用户开启自启动、电池优化白名单等系统设置
...
<p align="center">
  <img src="docs/screenshot_home.jpg" width="300" alt="SilentGuard 首页截图" />
</p>
...
6. 开启「启动守护」开关
7. 在首页可使用「一键求助」立即发送当前位置到监护邮箱
8. 在「记录」页查看短信、位置和邮件发送状态；在「轨迹」页查看位置轨迹
...
│   └── ServiceWatchdogReceiver.kt      # AlarmManager 兜底服务检查
├── util/
│   └── BackgroundGuideHelper.kt        # 国产 ROM 后台保活引导
└── ui/
...
| `SCHEDULE_EXACT_ALARM` | AlarmManager 兜底唤醒 |
| `WAKE_LOCK` | 定位期间防止 CPU 休眠 |

## 🧭 运行机制

- 设置页配置会自动保存；SMTP 授权码和高德 Web API Key 以加密方式存储。
- 守护服务采用**间歇式定位**：每周期执行一次性定位后立即释放 GPS，系统不会显示"持续使用定位"。
- **自适应间隔**：设备静止（连续多次位置未变化）时，定位间隔自动翻倍（最大2倍）；一旦检测到移动立即恢复正常间隔。
- **静止恢复保护**：静止超过 30 分钟会重置定位频率，避免设备移动后长时间无法感知。
- **夜间省电**：23:00-6:00 自动切换基站/WiFi 低功耗定位，不启用 GPS，间隔至少 30 分钟。
- **按需 WakeLock**：仅在定位执行期间（几秒）持有 CPU 唤醒锁，完成后立即释放。
- **异常未定位告警**：服务每 10 分钟检查一次最近成功定位时间，超过 `max(定位间隔 × 3, 30分钟)` 未成功定位时发送邮件，持续异常时最多每 2 小时重复提醒一次。
- **低电量提醒**：每次定位轮询后顺带读取系统电池状态，电量低于 10% 时发送邮件；持续低电量时最多每 6 小时重复提醒一次，电量恢复到 15% 后重置提醒状态。
- 位置记录默认执行 100 米距离去重，轨迹页自动记录和后台守护服务都会遵守该规则。
- 位置邮件按「邮件发送间隔」批量发送待发送位置点，成功后标记为已发送；每天 23:59 会额外发送当天位置汇总。
- 记录页支持按「全部 / 短信 / 位置 / 邮件」筛选，邮件页展示每次发送的成功或失败结果。
- **后台保活**：前台服务 + START_STICKY + AlarmManager 兜底检查 + onTaskRemoved 自动重启 + WorkManager 周期任务 + 开机自启动。
